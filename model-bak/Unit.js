...
var Unit = Class.extend({
	...
	Act: function(unit){
		if(unit != false && this.UnitInRange(unit)){
			this.DoDamage(unit);
		}
		else
			this.MoveTo();
	...
	Move: function(dir){
		...
		for(var i = 0; i < this.unitSize;i++)	 {
			...
			//if cell is occupied or on same team
			if(cellValue == 1 || (typeof window[cellValue] != "undefined" && window[cellValue].team == this.team)) {
				return false;
			} 
			/*else if(cellValue != 0) { //if cell is dif team and occupied
				//console.log(cellValue, this.id);
				this.DoDamage(window[cellValue]);
				return false;
			}*/
